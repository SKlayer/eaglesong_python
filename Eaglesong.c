...
void EaglesongPermutation( uint32_t *state ) {
    ...
    for(int i = 0 ; i < ROUND ; ++i ) {
        ...
        s0 ^= ROL32(s0, 2)^ROL32(s0, 4)^injection_constants[k]; ++k;
        s1 ^= ROL32(s1, 13)^ROL32(s1, 22)^injection_constants[k]; ++k;
        ROL_ADD(s0, s1);
        
        s2 ^= ROL32(s2, 4)^ROL32(s2, 19)^injection_constants[k]; ++k;
        s3 ^= ROL32(s3, 3)^ROL32(s3, 14)^injection_constants[k]; ++k;
        ROL_ADD(s2, s3);
        
        s4 ^= ROL32(s4, 27)^ROL32(s4, 31)^injection_constants[k]; ++k;
        s5 ^= ROL32(s5, 3)^ROL32(s5, 8)^injection_constants[k]; ++k;
        ROL_ADD(s4, s5);
        
        s6 ^= ROL32(s6, 17)^ROL32(s6, 26)^injection_constants[k]; ++k;
        s7 ^= ROL32(s7, 3)^ROL32(s7, 12)^injection_constants[k]; ++k;
        ROL_ADD(s6, s7);
        
        s8 ^= ROL32(s8, 18)^ROL32(s8, 22)^injection_constants[k]; ++k;
        s9 ^= ROL32(s9, 12)^ROL32(s9, 18)^injection_constants[k]; ++k;
        ROL_ADD(s8, s9);

        s10 ^= ROL32(s10, 4)^ROL32(s10, 7)^injection_constants[k]; ++k;
        s11 ^= ROL32(s11, 4)^ROL32(s11, 31)^injection_constants[k]; ++k;
        ROL_ADD(s10, s11);
        
        s12 ^= ROL32(s12, 12)^ROL32(s12, 27)^injection_constants[k]; ++k;
        s13 ^= ROL32(s13, 7)^ROL32(s13, 17)^injection_constants[k]; ++k;
        ROL_ADD(s12, s13);
        
        s14 ^= ROL32(s14, 7)^ROL32(s14, 8)^injection_constants[k]; ++k;
        s15 ^= ROL32(s15, 1)^ROL32(s15, 13)^injection_constants[k]; ++k;
        ROL_ADD(s14, s15);
    }
...
void EaglesongHash(uint8_t *output, uint8_t *input, int input_length) {
    ...
    int n = (((input_length + 1) << 3) + RATE - 1) / RATE;
    ...
    for(int i = 0, k = 0; ;) {
        for(int j = 0 ; j < M; ++j) {
            ((uint32_t *)output)[k] = htole32(state[j]);
            ++k;
        }

        ++i;
        if(i >= OUTPUT_LENGTH / LEN) {
            break;
        }
        EaglesongPermutation(state);
    }
